...
assert Part3{
	all ad : AbstractDiagram | (#Heat = 1 and #Dry = 0 and #Delay = 0 and 
								MCSB[ad] and HSB[ad] and TSB[ad] and DSB[ad]) implies
		(ad.Comps = MainController + Heater) and 
		(ad.Sockets = dri + hri + tri) and
		(ad.Balls = dpi) and 
		(ad.SocketPorts = MainController -> dri) and 
		(ad.BallPorts = Heater -> dpi) and
		(ad.DelegatedLink = cd) 
} check Part3 for 4 but exactly 1 AbstractDiagram
...
}run P2 for 4 but exactly 1 AbstractDiagram
